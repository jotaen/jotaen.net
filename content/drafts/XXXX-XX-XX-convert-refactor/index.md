...
try {
  if (process.argv.length === 4) {
    ...
    if (["-bin", "-hex", "-dec"].includes(target) &&
        /^(0b[01]+|0x[0-9a-fA-F]+|\d+)$/.test(input)) {
      let decimal;
...
try {
  if (process.argv.length === 4) {
    ...
    if (options.includes(target) && numberShape.test(input)) {
      console.log(convert(target, input));
    } else {
      throw "Input arguments invalid";
    }
  } else {
...
try {
  ...
  const result = convert(input, target);
  console.log(result);
} catch(e) {
  console.log("Error: " + e);
  process.exit(1);
...
## #4. Variable declaration is initialisation
...
// core logic
const convert = (target, input) => {
  const decimal = (input.substr(0, 2) === "0b") ?
    parseInt(input.substr(2), 2) :
    parseInt(input);
  if (target === "-bin") return "0b" + decimal.toString(2);
  else if (target === "-hex") return "0x" + decimal.toString(16);
  else return decimal.toString();
...
## #5. Minimise cyclomatic complexity
...
const convert = (target, input) => {
  const decimal = (input.substr(0, 2) === "0b") ?
    parseInt(input.substr(2), 2) :
    parseInt(input);
  return targetConverter[target](decimal);
};
```
...
const convert = (target, input) => {
  const decimal = (input.substr(0, 2) === "0b") ?
    parseInt(input.substr(2), 2) :
    parseInt(input);
  const converter = converterDict[target]
  return converter.prefix + decimal.toString(converter.base);
};
```

## #7. Open-closed principle

The problem of redundancies is not solved yet. This is not immediately obvious anymore, but it becomes clear when you imagine we wanted to add a fourth conversion to the programm. This would require us to touch 4 different places at the moment: `converterDict`, the initialisation of `decimal`, plus `numberShape` and `options` that we extracted earlier – needing to keep all of them in sync is an avoidable risk, plus it puts a big burden on future maintainers of our application.
...
const plugins = {
  ...
  "-dec": {prefix: "", base: 10, shape: /^[0-9]+$/},
};

const convert = (sourcePlugin, target, input) => {
  const decimal = parseInt(
    input.substr(sourcePlugin.prefix.length),
    sourcePlugin.base
  );
...
try {
  ...
  const target = process.argv[2];
  const input = process.argv[3];
  const sourcePlugin = Object.values(plugins).find(p => p.shape.test(input));
  if (!Object.keys(plugins).includes(target) || !sourcePlugin) {
    throw "Input arguments invalid";
  }
  const result = convert(sourcePlugin, target, input);
  console.log(result);
} catch(e) {
  console.log("Error: " + e);
  process.exit(1);
...
const convert = (target, input) => {
  const targetPlugin = plugins[target];
  const inputPlugin = Object.values(plugins).find(n => n.shape.test(input));
  if (!targetPlugin || !inputPlugin) {
    throw "Input arguments invalid";
  }
  const decimal = parseInt(
...
- It is easier to extend: similar concepts are grouped together and segregated by clear interfaces.
